...
Csig = 3.093 - 1.029*llr_mean + 0.603*pesq_mos-0.009*wss_dist;
    Csig = max(1, Csig); Csig = min(5, Csig); %% adding for fitting range 1 to 5
Cbak = 1.634 + 0.478 *pesq_mos - 0.007*wss_dist + 0.063*segSNR;
    Cbak = max(1, Cbak); Cbak = min(5, Cbak); %% adding for fitting range 1 to 5
Covl = 1.594 + 0.805*pesq_mos - 0.512*llr_mean - 0.007*wss_dist;
    Covl = max(1, Covl); Covl = min(5, Covl); %% adding for fitting range 1 to 5
